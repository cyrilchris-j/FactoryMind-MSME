...
function buildFactoryContext(): string {
  const kpis = getDashboardKPIs();
  const hero = getHeroMetrics();
  const lowStock = inventory.filter((i) => i.quantity <= i.reorderLevel);
  const underutilized = machines.filter((m) => m.utilization < 50 && m.status === 'running');
  const atRisk = machines.filter((m) => m.healthScore < 60);
  const topCustomers = [...customers].sort((a, b) => b.profit - a.profit).slice(0, 5);
  const overloadedWorkers = workers.filter((w) => w.overtime > 8);

  return `
FACTORY: ${factory.name}, ${factory.location}
Industry: ${factory.industry} | Employees: ${factory.employeeCount} | Machines: ${machines.length}
...
${machines.slice(0, 20).map((m) => `${m.name}: ${m.status}, ${m.utilization}% util, health ${m.healthScore}%`).join('\n')}
...
Present: ${workers.filter((w) => w.attendance === 'present').length}
...
ENERGY (Latest Month):
Consumption: ${energyRecords[energyRecords.length - 1]?.totalConsumption} kWh
Cost: ₹${energyRecords[energyRecords.length - 1]?.cost.toLocaleString('en-IN')}

PRODUCTION ORDERS:
Active: ${productionOrders.filter((o) => o.status === 'in_progress').length}
...
function generateMockResponse(query: string): string {
  ...
  const hero = getHeroMetrics();
  const underutilized = machines.filter((m) => m.utilization < 50);
  const atRisk = machines.filter((m) => m.healthScore < 60);
  const lowStock = inventory.filter((i) => i.quantity <= i.reorderLevel);
  const topCustomer = [...customers].sort((a, b) => b.profit - a.profit)[0];
  const overloaded = workers.filter((w) => w.overtime > 8);
...
|----------|---------|--------|--------|
${[...customers].sort((a, b) => b.profit - a.profit).slice(0, 5).map((c) => `| ${c.name} | ₹${(c.revenue / 100000).toFixed(1)}L | ₹${(c.profit / 100000).toFixed(1)}L | ${((c.profit / c.revenue) * 100).toFixed(1)}% |`).join('\n')}
...
${['Production', 'Assembly', 'Quality', 'Maintenance'].map((d) => {
  const deptWorkers = workers.filter((w) => w.department === d);
...
**${factory.name}** is operating at **${hero.overallEfficiency}% efficiency** today.

**Key Highlights:**
- Machine Health: ${hero.machineHealthScore}% average across ${machines.length} machines
...
- ${productionOrders.filter((o) => o.status === 'delayed').length} delayed production orders
...
export function getWeeklySummary() {
  ...
  return {
    ...
    highlights: [
      ...
      `${maintenanceRecords.filter((m) => m.status === 'completed').length} maintenance tasks completed on schedule`,
      `Revenue grew 15.3% with ${salesOrders.filter((o) => o.status === 'delivered').length} orders delivered`,
    ],
    concerns: [
      `${inventory.filter((i) => i.quantity <= i.reorderLevel).length} inventory items below reorder level`,
      `${machines.filter((m) => m.healthScore < 60).length} machines showing declining health scores`,
      `${workers.filter((w) => w.attendance === 'absent').length} attendance issues this week`,
    ],
...
export function getDailyBriefing() {
  ...
  return {
    ...
    aiInsight: `Factory is operating at ${hero.overallEfficiency}% efficiency. Focus on completing delayed orders and addressing ${inventory.filter((i) => i.quantity <= i.reorderLevel).length} low-stock items to maintain production flow.`,
  };
}
