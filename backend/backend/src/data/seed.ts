...
const STATUSES: Machine['status'][] = ['running', 'running', 'running', 'idle', 'maintenance', 'offline'];
...
function generateSparkline(length = 7): number[] {
...
export const machines: Machine[] = Array.from({ length: 50 }, (_, i) => {
...
export const workers: Worker[] = Array.from({ length: 100 }, (_, i) => ({
  ...
  shift: (['morning', 'afternoon', 'night'] as const)[i % 3],
...
export const productionOrders: ProductionOrder[] = Array.from({ length: 500 }, (_, i) => {
  const qty = randomBetween(100, 5000);
  const completed = randomBetween(0, qty);
  const status: ProductionOrder['status'] =
    completed === qty ? 'completed' : completed === 0 ? 'pending' : Math.random() > 0.85 ? 'delayed' : 'in_progress';
  return {
    ...
    completed,
    status,
    priority: (['low', 'medium', 'high', 'critical'] as const)[randomBetween(0, 3)],
...
export const maintenanceRecords: MaintenanceRecord[] = Array.from({ length: 120 }, (_, i) => ({
  id: `MNT-${String(i + 1).padStart(4, '0')}`,
  machineId: machines[i % machines.length].id,
  type: (['preventive', 'corrective', 'predictive'] as const)[i % 3],
  ...
  technician: workers[randomBetween(0, 14)].name,
  downtime: randomBetween(1, 48),
}));

export const energyRecords: EnergyRecord[] = Array.from({ length: 12 }, (_, i) => {
  ...
  return {
    ...
    cost: total * 8.5,
    machineBreakdown: machines.slice(0, 10).map((m) => ({
      machineId: m.id,
...
export const salesOrders: SalesOrder[] = Array.from({ length: 500 }, (_, i) => {
  ...
  return {
    ...
    status: (['pending', 'confirmed', 'shipped', 'delivered'] as const)[randomBetween(0, 3)],
...
export function getDashboardKPIs(): DashboardKPI[] {
  ...
  const avgUtilization = Math.round(machines.reduce((s, m) => s + m.utilization, 0) / machines.length);
  ...
  const todayProduction = productionOrders.filter((o) => o.status === 'in_progress').reduce((s, o) => s + o.completed, 0);
  ...
  const avgHealth = Math.round(machines.reduce((s, m) => s + m.healthScore, 0) / machines.length);
  const energyCost = energyRecords[energyRecords.length - 1]?.cost || 0;
  ...
  return [
    ...
    { label: 'Active Orders', value: productionOrders.filter((o) => o.status === 'in_progress').length, trend: -2.4, trendDirection: 'down', sparkline: generateSparkline() },
    ...
    { label: 'Worker Attendance', value: presentWorkers, unit: `/ ${workers.length}`, trend: -3, trendDirection: 'down', sparkline: generateSparkline() },
...
export function getHeroMetrics() {
  const avgHealth = Math.round(machines.reduce((s, m) => s + m.healthScore, 0) / machines.length);
  const avgUtilization = Math.round(machines.reduce((s, m) => s + m.utilization, 0) / machines.length);
  const todayProduction = productionOrders.filter((o) => o.status === 'in_progress').reduce((s, o) => s + Math.min(o.completed, o.quantity * 0.1), 0);
  const energyToday = Math.round(energyRecords[energyRecords.length - 1]?.totalConsumption / 30 || 2000);
...
export function getAnalyticsData() {
  ...
  return {
    ...
    energyTrend: months.map((month, i) => ({
      month,
      consumption: energyRecords[i]?.totalConsumption || randomBetween(45000, 75000),
      cost: energyRecords[i]?.cost || randomBetween(380000, 640000),
    })),
    ...
    machineUtilization: machines.slice(0, 15).map((m) => ({
      name: m.name.split(' #')[0],
...
export const demoUsers = [
  { id: 'user-1', email: 'owner@kumar-precision.com', password: 'demo123', name: 'Rajesh Kumar', role: 'factory_owner' as const },
  { id: 'user-2', email: 'production@kumar-precision.com', password: 'demo123', name: 'Priya Sharma', role: 'production_manager' as const },
  { id: 'user-3', email: 'maintenance@kumar-precision.com', password: 'demo123', name: 'Amit Patel', role: 'maintenance_engineer' as const },
  { id: 'user-4', email: 'inventory@kumar-precision.com', password: 'demo123', name: 'Sneha Reddy', role: 'inventory_manager' as const },
  { id: 'user-5', email: 'admin@kumar-precision.com', password: 'demo123', name: 'Admin User', role: 'admin' as const },
];
